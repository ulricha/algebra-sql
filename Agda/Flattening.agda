...
data Env : {n : ℕ} → Sig n → Set where
  []  : Env []
  _∷_ : {n : ℕ} {Γ : Sig n} {τ : Type} → ⟦ τ ⟧ → Env Γ → Env (τ ∷ Γ)

envLookup : {n : ℕ} {Γ : Sig n} (i : Fin n) → Env Γ → ⟦ lookup i Γ ⟧
...
interpret : {n : ℕ} {Γ : Sig n} {τ : Type} → Env Γ → FExpr Γ τ → ⟦ τ ⟧
interpret env (capp e₁ e₂)  = interpret env e₁ (interpret env e₂) -- NEW
interpret env (lapp e₁ e₂)  = L.zipWith id (interpret env e₁) (interpret env e₂) -- NEW
interpret env (clos e₁ e₂)  = λ x → interpret (x ∷ env) e₁ -- NEW
interpret (n ∷ env) (lclos e₁ e₂) = {!!}
...
mutual
  ...
  ↑Expr (cmap e₁ e₂)    = unconc (↑Expr e₂) (lapp (conc (ldist (↑Expr e₁) (↑Expr e₂))) (conc (↑Expr e₂)))
