module Flattening-Simple where
...
mutual
  lift : {n : ℕ} {Γ : Sig n} {τ τ′ : Type} → FEnv (↑Sig Γ) → PA τ′ → Expr (τ′ ∷ Γ) τ → F⟦ ↑ τ ⟧
  lift env s (app e₁ e₂)   = (proj₂ (lift env s e₁)) (lift env s e₂)
  lift {τ′ = τ} env s (lam e) = lengthPA τ s , λ ys → lift (s ∷ env) ys e
  lift env s (add e₁ e₂)   = L.zipWith _+_ (lift env s e₁) (lift env s e₂)
  lift env s (pair e₁ e₂)  = lift env s e₁ , lift env s e₂
  lift {τ′ = τ} env s (con int x)   = L.replicate (lengthPA τ s) x
  lift {τ′ = τ} env s (con (list int) xs) = L.replicate (lengthPA τ s) (length xs) , L.concat (L.replicate (lengthPA τ s) xs)
  lift env s (con _ xs)    = {!!} -- this one is probably unproblematic
  lift env s (var zero)    = s
  lift env s (var (suc i)) = subst id (lemma i _) (envLookup i env)
  lift env s (cmap {τ₁} {τ₂} e₁ e₂)
                           = let xs = lift env s e₂
                                 f  = proj₂ (lift env s e₁)
                             in unconc {τ₂} {τ₁} xs (f (conc {τ₁} xs))

  liftEnv : {n : ℕ} {Γ : Sig n} → FEnv Γ → FEnv (↑Sig Γ)
  liftEnv []       = []
  liftEnv (x ∷ xs) = {!!} -- this one looks critical, and can probably be used to justify closures

  -- Flattening is an alternative interpretation.
  flat : {n : ℕ} {Γ : Sig n} {τ : Type} → FEnv Γ → Expr Γ τ → F⟦ τ ⟧
  flat env (app e₁ e₂)  = proj₁ (flat env e₁) (flat env e₂)
  flat env (lam e)      = (λ x → flat (x ∷ env) e) , λ xs → lift (liftEnv env) xs e
  flat env (add e₁ e₂)  = flat env e₁ + flat env e₂
  flat env (pair e₁ e₂) = flat env e₁ , flat env e₂
  flat env (con c x)    = flatConst c x
  flat env (cmap e₁ e₂) = proj₂ (flat env e₁) (flat env e₂)
